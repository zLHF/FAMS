...
def create_role():
    ...
    if Role.query.filter_by(tenant_id=_current_tenant_id(), name=data["name"]).first():
        return jsonify({"error": "角色名称已存在"}), 400
    if Role.query.filter_by(tenant_id=_current_tenant_id(), code=data["code"]).first():
...
def assign_permissions(id):
    ...
    for pid in perm_ids:
        db.session.execute(role_permissions.insert().values(role_id=role.id, permission_id=pid))
    db.session.commit()
