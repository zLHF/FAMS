...
def test_distribute_flow(client, db, app):
    with app.app_context():
        ...
        db.session.add(Asset(code="F001", name="流转测试", status="idle"))
        db.session.commit()
    t = get_token(client)
...
def test_borrow_return_flow(client, db, app):
    with app.app_context():
        uid, _ = setup_admin(db)
        db.session.add(Asset(code="B001", name="借用测试", status="idle"))
        db.session.commit()
    t = get_token(client)
...
def test_owner_change_flow(client, db, app):
    with app.app_context():
        ...
        db.session.add(Asset(code="C001", name="变更测试", status="distributed", owner_id=uid))
        db.session.commit()
        uid2 = user2.id
    t = get_token(client)
